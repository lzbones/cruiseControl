...
* **按键指令**：直接映射为增速 ($I_0$)、降速 ($I_1$)、取消 ($I_2$)、退出 ($I_4$)。
* **刹车踏板踩下**：作为紧急干预输入，直接映射为 **刹车 ($I_3$)** 指令。
* **人工油门踩下**：体现在系统子态的 $b_1$ 位（$b_1=1$ 代表有油， $b_1=0$ 代表无油），影响系统状态的判定。例如，在待命状态下踩油门会使系统处于“有油待命 ($S_4$)”状态，此时按键将触发以当前实际车速为目标的控制。

#### 1.1.3 CC 系统决策的关系表 (第一张表 - 表 1)
当输入不同的驾驶指令时，系统根据当前的运行状态 $S_n$，决策出相应的响应动作 $R_k$：

| 驾驶指令 \ 系统状态 | $S_0$ (适速在控) | $S_1$ (高速在控) | $S_2$ (适速无油有史待命) | $S_3$ (适速无油无史待命) | $S_4$ (有油待命) | $S_5$ (非适速待命) | $S_6$ (低速在控) |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **增速 ($I_0$)** | $R_1$ (目标增量) | $R_3$ (保持控制) | $R_4$ (继承控制) | $R_5$ (无继控制) | $R_5$ (无继控制) | $R_6$ (系统待命) | $R_7$ (报错待命) |
| **降速 ($I_1$)** | $R_2$ (目标减量) | $R_3$ (保持控制) | $R_6$ (系统待命) | $R_5$ (无继控制) | $R_5$ (无继控制) | $R_6$ (系统待命) | $R_7$ (报错待命) |
| **取消 ($I_2$)** | $R_6$ (系统待命) | $R_6$ (系统待命) | $R_6$ (系统待命) | $R_6$ (系统待命) | $R_6$ (系统待命) | $R_6$ (系统待命) | $R_6$ (系统待命) |
| **刹车 ($I_3$)** | $R_6$ (系统待命) | $R_6$ (系统待命) | $R_6$ (系统待命) | $R_6$ (系统待命) | $R_6$ (系统待命) | $R_6$ (系统待命) | $R_6$ (系统待命) |
| **退出 ($I_4$)** | $R_8$ (系统退出) | $R_8$ (系统退出) | $R_8$ (系统退出) | $R_8$ (系统退出) | $R_8$ (系统退出) | $R_8$ (系统退出) | $R_8$ (系统退出) |

> [!NOTE]  
> **低速在控主动安全保护**：  
> 若系统处于 $S_6$（低速在控）状态且持续时间超过阈值 $t_{timeout}$（$3.0\text{s}$），即使无任何驾驶指令输入（`COMMAND_NONE`），系统也将主动触发并执行 **$R_7$ (报错待命)**。

#### 1.1.4 系统状态判定表 (第二张表 - 表 2)
系统根据车速子态 $S_{vi}$ 与系统子态集 $S_{sj}$ 进行同类项合并后的系统状态 $S_n$ 判定关系如下：
